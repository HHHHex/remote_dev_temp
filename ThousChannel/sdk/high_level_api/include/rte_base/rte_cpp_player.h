 ...
 */
  ...
  Player(Player &&other):c_player(other.c_player) {
    other.c_player = {};
  };
  ...
  void OpenWithStream(Stream* stream, std::function<void(rte::Error* err)> cb)  {
    CallbackContext<Player>* callbackCtx = new CallbackContext<Player>(this, cb);
    RtePlayerOpenWithStream(&c_player, stream != nullptr ? &stream->c_stream : nullptr, &CallbackFunc<::RtePlayer, Player>, callbackCtx);
  };
