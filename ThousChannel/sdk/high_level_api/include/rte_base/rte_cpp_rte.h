...
class Config {
 public:
  ...
  /**
   * Set whether to use string user ID
   * @technical preview
   * @param use_string_uid true to use string user ID, false to use integer user ID
   * @param err 
   */
  void SetUseStringUid(bool use_string_uid, Error *err = nullptr) {
    RteConfigSetUseStringUid(&c_rte_config, use_string_uid, err != nullptr ? err->get_underlying_impl() : nullptr);
  }

  /**
   * Get whether string user ID is used
   * @technical preview
   * @param err 
   * @return bool true if using string user ID, false if using integer user ID
   */
  bool GetUseStringUid(Error *err = nullptr) {
    bool use_string_uid;
    RteConfigGetUseStringUid(&c_rte_config, &use_string_uid, err != nullptr ? err->get_underlying_impl() : nullptr);
    return use_string_uid;
  }
 ...
 */
 ...
 private:
  ...
  friend class MicAudioTrack;
  friend class LocalRealTimeStream;
  friend class LocalCdnStream;
  friend class RemoteStream;
  friend class RemoteRealTimeStream;
  friend class CameraVideoTrack;
