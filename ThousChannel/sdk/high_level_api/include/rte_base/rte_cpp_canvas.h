...
class Canvas {
 public:
  ...
  Canvas(const Canvas& other){
    c_canvas.handle = other.c_canvas.handle;
    is_owner_ = false;
  }

  Canvas(Canvas&& other) : c_canvas(other.c_canvas), is_owner_(other.is_owner_) {
    other.c_canvas = {};
    other.is_owner_ = false;
  }

  Canvas(RteCanvas *canvas) {
    if(canvas == nullptr) {
      return;
    }
    c_canvas.handle = canvas->handle;
    is_owner_ = false;
  }
   ...
   */
  bool AddView(View *view, ViewConfig *config, Error *err = nullptr) {
